...
<html>
	...
	<body style = "background-color: grey; font-family: 'assistant';">
		...
		<div class = "panel-side">
			<div class = "one">
				<a href = "http://localhost/cs320/student1.php"><img src = signout.png></a>
			</div>
		<div class = "panel-top">
			<?php
		...
		<?php 
			if(isset($_GET['StudNum']) && isset($_GET['Name']) && isset($_GET['Sem']) && isset($_GET['SY'])) 
			{
			...
			function build_table($result){
				if(mysqli_num_rows($result) > 0){
					...
					while($row = mysqli_fetch_array($result)){
					    echo "<tr>";
					    //echo "<tdstyle='display:none';>" . $row['StudNum'] . "</td>";
			...
			$sql = "SELECT * FROM tblgrd WHERE StudNum = ? AND semester = ? AND SY = ? ORDER BY SubjCode asc";
			if($stmt = mysqli_prepare($link, $sql)){
				mysqli_stmt_bind_param($stmt,"sss",$StudNum,$Sem,$SY);
				...
				echo"<center>Error on grades load</center>"; 
			}
